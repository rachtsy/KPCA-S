...
#                     wandb.log({"min_pi1": pi1_val.min()})
            ...
            if args.dataset in ['enwik8', 'text8']:
                ...
                if use_wandb:
                    wandb.log({"ppl": math.exp(cur_loss),"epoch":train_step})
        ...
        if train_step % args.eval_interval == 0:
            ...
            if args.dataset in ['enwik8', 'text8']:
                ...
                if use_wandb:
                    wandb.log({"valid_ppl": math.exp(val_loss),"epoch":train_step})
            ...
            if not best_val_loss or val_loss < best_val_loss:
                if not args.debug:
                    with open(os.path.join(args.work_dir,
                                           f'model){log_start_time}.pt'), 'wb') as f:
                        torch.save(model, f)
                    with open(os.path.join(args.work_dir,
                                           f'optimizer_{log_start_time}.pt'), 'wb') as f:
...
with open(os.path.join(args.work_dir, f'model_{log_start_time}.pt'), 'rb') as f:
    model = torch.load(f)
para_model = model.to(device)
