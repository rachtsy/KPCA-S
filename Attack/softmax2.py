...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., robust=False, layerth=0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.robust and self.layer==0: 
            ...
            for i in range(0,1):
                ...
                # L = l.norm(p='nuc',dim=[-1,-2])
                # S1 = lambd*s.norm(p=1,dim=[-1,-2])
                # obj = L+S1
                # print(l.norm(p='nuc',dim=[-1,-2])[0,0])
                # print(s.norm(p=1,dim=[-1,-2])[0,0])
                # wandb.log({'obj mean': obj[0,0], 'L': L[0,0], 'S': S1[0,0], 'iteration': i})
                y = y+mu*(k-l-s)
            ...
            l = l.softmax(dim=-1)
            l = self.attn_drop(l)
            # obj = l.norm(p='nuc',dim=[-1,-2])+lambd*s.norm(p=1,dim=[-1,-2])
            # wandb.log({'obj mean': obj[0,0], 'iteration': i})
            x = l @ v
            y = y+mu*(k-x-s)

            if not self.printed:
                print("lambda = "+str(lambd))
                print("iterations = "+str(i+2))
                self.printed = True

            if torch.sum(y) == 0:
                wandb.log({'mu': mu, 's':s.mean(), 'y':y[0,0,0,0]})
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, layerth = None, robust=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
                                    attn_drop=attn_drop, proj_drop=drop, robust=robust, layerth=layerth)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, layerth = i, robust=robust)
