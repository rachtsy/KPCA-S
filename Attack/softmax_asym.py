...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., layerth=0):
        ...
        # self.v = np.zeros((1,1,dim))
        # self.q = np.zeros((1,1,dim))
        # self.k = np.zeros((1,1,dim))
    ...
    def forward(self, x):
        ...
        # self.v = v.permute(0,2,1,3).reshape(B,N,C).cpu().numpy()
        # self.q = q.permute(0,2,1,3).reshape(B,N,C).cpu().numpy()
        # self.k = k.permute(0,2,1,3).reshape(B,N,C).cpu().numpy()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',pretrained_cfg=None,pretrained_cfg_overlay=None,robust=False):
        """
        Args:
        ...
        # self.v = np.zeros((1,num_patches+self.num_tokens,self.embed_dim))
        # self.q = np.zeros((1,num_patches+self.num_tokens,self.embed_dim))
        # self.k = np.zeros((1,num_patches+self.num_tokens,self.embed_dim))
    ...
    def forward_features(self, x):
        ...
        x = self.blocks(x)
        x = self.norm(x)
        # self.v = np.zeros((x.shape[0],x.shape[1],self.embed_dim))
        # self.q = np.zeros((x.shape[0],x.shape[1],self.embed_dim))
        # self.k = np.zeros((x.shape[0],x.shape[1],self.embed_dim))
        ...
        #     if self.v.flatten().sum() == 0:
        #         self.v = np.array([block.attn.v])
        #     else:
        #         self.v = np.append(self.v,[block.attn.v],axis=0)
        #     if self.q.flatten().sum() == 0:
        #         self.q = np.array([block.attn.q])
        #     else:
        #         self.q = np.append(self.q,[block.attn.q],axis=0)
        #     if self.k.flatten().sum() == 0:
        #         self.k = np.array([block.attn.k])
        #     else:
        #         self.k = np.append(self.k,[block.attn.k],axis=0)
